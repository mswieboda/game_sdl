...
- **Visual Verification:** For GUI/UI examples, perform automated visual checks on macOS. Run the example with a short `timeout` in the background, sleep for 0.8 seconds to allow the window to render and receive focus, query the active window ID, take a window-specific screen capture (playing the sound so the user hears it, and excluding drop shadows) to your conversation's brain/artifacts directory, and call `wait` to cleanly exit:
  - Command:
    ```bash
    timeout 2 ./build/foo &
    sleep 0.8
    screencapture -o -l "$(get_window_id --active)" /Users/matt/.gemini/antigravity-cli/brain/<conversation-id>/test_ui_screenshot.png
    wait
    ```
